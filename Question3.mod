...
# Major proportion per team
param major_limit;
# Gender proportion per team
param gender_limit;
# Happiness Parameter
param happiness;
...
subject to genderproportionconstraint{p in PROJ, k in GENDERS}:
	sum{i in STUDS} gender[i,k]*s[i,p] <= gender_limit * sum{i in STUDS} s[i,p];